...
export const createInitialPlayer = (name: string): Player => { // Removed playerClassInput parameter
  const playerClass = PlayerClass.HERO; // Always Hero
  ...
  try {
    ...
    if (savedData) {
      ...
      if (parsedData.player && parsedData.regions) {
         ...
         if (loadedPlayer.playerClass !== PlayerClass.HERO) { // Check if the class is not HERO
            console.warn(`旧バージョンの職業(${loadedPlayer.playerClass})を${PlayerClass.HERO}に変換しました。`);
            loadedPlayer.playerClass = PlayerClass.HERO; // Convert to HERO
         }
...
export const loadFromPassword = (password: string): { player: Player | null; regions: Record<string, Region> } => {
  ...
  try {
    const binaryString = atob(password);
    ...
    try {
        ...
        const decodedString = decodeURIComponent(atob(password)); 
...
export const getEnemiesForEncounter = (region: Region, encounterIndex: number): Enemy[] => {
  let enemyIds: string[];
