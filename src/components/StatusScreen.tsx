...
export const StatusScreen: React.FC<StatusScreenProps> = ({ player, setGamePhase, onEquipItem, onUnequipItem, onEnhanceEquipment }) => {
  ...
  const displayExperience = player.level < XP_FOR_LEVEL.length ? `${player.experience} / ${xpToNextLevel}` : `${player.experience} (最大)`;
  const nextLevelProgress = player.level < XP_FOR_LEVEL.length ? `あと ${Math.max(0, xpToNextLevel - player.experience)} でレベルアップ` : "レベル最大";
